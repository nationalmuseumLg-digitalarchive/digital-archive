import { notFound } from 'next/navigation'
import config from '@payload-config'
import { getPayload } from 'payload'
import React, {cache} from 'react'
import { RenderBlocks } from '@/utils/RenderBlocks'
import PaginatedBlocks from '@/components/PaginatedBlocks'
import * as motion from "framer-motion/client"


// Set revalidation period to 60 seconds to balance speed and freshness
export const revalidate = 60

const Page =  async({params}) => {

  const {slug} = await params

   const page = await queryPageBySlug({
        slug
    })

    if (!page) {
        return notFound()
    }

    // Use ethnographicItems collection for ethnographic_objects, blocks for others
    const isEthnographic = page.slug === 'ethnographic_objects'
    
    // Fetch all ethnographic items directly from collection if this is the ethnographic page
    let itemsToRender = page.pageSection?.layout
    if (isEthnographic) {
      const payload = await getPayload({ config })
      const ethnographicResult = await payload.find({
        collection: 'ethnographicItems',
        limit: 1000,
        depth: 2,
        select: {
          objectName: true,
          image: true,
          description: true,
          accessionNumber: true,
          location: true,
        },
      })
      itemsToRender = ethnographicResult.docs
    }

    const anim = {
      initial: {
        width: '100vw',
        // x: '100vw'
      },
      open: {
        width: '0',
        // x: 0
      },
      closed: {
        width: '100vw',
      },
    }

    return (
      <article>
        <div className="w-full h-[100%] font-montserrat flex flex-col gap-4 p-8 overflow-hidden">
          <h1 className="sm:text-[3rem] text-[2rem] font-bold pb-8 uppercase">
            {page.internalName}
          </h1>

          <div className="w-[100%] flex justify-center gap-2 h-[100%]">
            {/* <RenderBlocks blocks={page.pageSection.layout}/> */}
            <PaginatedBlocks blocks={itemsToRender} itemsPerPage={20} isCollection={isEthnographic} />
          </div>
        </div>

        <motion.div
          variants={anim}
          initial="initial"
          animate="open"
          exit="closed"
          transition={{
            duration: 0.4,
            ease: 'easeOut',
          }}
          className="w-[100vw] bg-black h-[100vh] top-0 absolute"
        ></motion.div>
      </article>
    )
  }

  export default Page

  const queryPageBySlug = cache(async ({ slug }) => {
    const parsedSlug = decodeURIComponent(slug)

    const payload = await getPayload({ config })

    const result = await payload.find({
      collection: 'alternativePages',
      limit: 1,
      where: {
        slug: {
          equals: parsedSlug,
        },
      },
      select: {
        internalName: true,
        pageSection: true,
        slug: true,
      },
      depth: 2,
    })

    return result.docs?.[0] || null
  })

  export const generateStaticParams = async () => {
    const payload = await getPayload({ config })

    const pages = await payload.find({
      collection: 'alternativePages',
      draft: false,
      limit: 5,
      select: {
        slug: true,
      },
    })

    return pages.docs?.map((doc) => ({ slug: doc.slug }))
  }
